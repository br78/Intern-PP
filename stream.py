...
# Load the dataset (first 50,000 rows to save memory)
@st.cache_data
def load_data():
...
st.title("Medicare Data")

# Sidebar filters
st.sidebar.header("Filter Options")

states = df['Prscrbr_State_Abrvtn'].dropna().unique()
selected_states = st.sidebar.multiselect("Select States", sorted(states), default=list(states)[:5])

specialties = df['Prscrbr_Type'].dropna().unique()
selected_specialties = st.sidebar.multiselect("Select Specialties", sorted(specialties), default=list(specialties)[:5])

min_claims, max_claims = int(df['Tot_Clms'].min()), int(df['Tot_Clms'].max())
claims_range = st.sidebar.slider("Total Claims Range", min_claims, max_claims, (min_claims, max_claims))
...
fig1 = px.bar(state_claims, x='Prscrbr_State_Abrvtn', y='Tot_Clms', color='Tot_Clms', labels={'Tot_Clms': 'Total Claims'})
st.plotly_chart(fig1, use_container_width=True)

# Visualization - Top Prescribers by Drug Cost
st.subheader("Top 10 Prescribers by Total Drug Cost")
top_prescribers = filtered_df.nlargest(10, 'Tot_Drug_Cst')
fig2 = px.bar(top_prescribers, x='Prscrbr_Last_Org_Name', y='Tot_Drug_Cst', color='Tot_Drug_Cst', labels={'Tot_Drug_Cst': 'Drug Cost'})
st.plotly_chart(fig2, use_container_width=True)

# Now use the fully filtered data
...
st.caption("Data Source: Medicare Prescriber Public Data")